...
- **Customer Search API**: Now fully paginated, returns total count, and all errors fixed. Query performance improved for large datasets.
- **Frontend Pagination & Debounce**: Customer search UI now uses debounce and pagination controls for faster, smoother results. Pagination UI is placed near search results.
- **Redundancy Removal**: Unused state and duplicate logic removed across main pages.
- **Error Fixes**: All critical backend and frontend errors resolved.

### How to Use
- Customer search now supports fast, paginated queries. Use the search bar and pagination controls to navigate results.
- Backend APIs for users and lessons are also paginated.

### Technical Details
- Next.js (App Router), React, TypeScript, Tailwind CSS, Prisma ORM, PostgreSQL, JWT, Vercel, Turbopack
- API endpoints: `/api/customers/search`, `/api/users/search`, `/api/lessons/search` (all paginated)
- State management: React hooks, debounce for search
- UI: Pagination controls near search results
